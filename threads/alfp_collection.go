...
func processCoreEpochCollection(epochData *structures.CoreEpochData) {
	...
	// Proactive collection is intentionally disabled for the very first core epoch.
	//
	// Rationale: per-leader scheduling needs an authoritative epoch start timestamp
	// that comes from core (the StartTimestamp embedded in NextEpochDataHandler of
	// the AggregatedEpochRotationProof for the *previous* epoch). For epoch 0 there
	// is no previous epoch and therefore no such proof. Falling back to a static
	// genesis offset proved fragile (it drifts from real core progress and made the
	// anchor request finalization for a leader that is still actively producing
	// blocks, locking in a low VotingStat.Index in modulr-core's first-write-wins
	// FINALIZATION_THREAD_METADATA store and stalling LMF). For epoch 0 we rely
	// on the normal LFT POST path to deliver ALFPs.
	if epochData.EpochId == 0 {
		return
	}
...
//
// epochStart is taken from the AggregatedEpochRotationProof generated by the
// PREVIOUS core epoch (the proof is stored under its NextEpochId, which equals
// this epoch's id). That proof carries NextEpochStartTimestamp — the canonical
// scheduled start time the modulr-core approvement thread itself uses for
// EpochStillFresh and leaderTimeIsOut. Using the same source of truth keeps the
// anchor's collection cadence in lockstep with core: even if core ticks slowly,
// EpochStillFresh and our derived leader-end timestamps share the same
// reference point, so we never request finalization for a leader the core
// network still considers active (epoch 0 is handled separately by the caller).
//
// Returns nil when the necessary proof is not yet available locally; the caller
// will simply skip this tick and retry later.
func computeLeaderEndTimestamps(epochId int, leadersCount int) []int64 {
	...
	// Stored under NextEpochId, so the proof for "previous -> this" is keyed by epochId.
	rotationProof := utils.LoadAggregatedEpochRotationProof(epochId)
	if rotationProof == nil || rotationProof.NextEpochId != epochId {
		return nil
	}

	epochStart := int64(rotationProof.EpochData.NextEpochStartTimestamp)
	if epochStart <= 0 {
		return nil
	}
